							...
							<div className={`flex flex-col gap-2 px-5 py-5`}>
								...
								<div className={`flex flex-col items-start`}>
									...
									<div className={`flex items-center`}>
										<div className={`w-50 h-50 ${caller === 'earthline-made' ? '' : 'bg-white'} rounded-xl flex items-center justify-center p-5`}>
											<ResponsiveImage className={`${caller === 'earthline-made' ? 'rounded-full' : ''}`} src={caller === "earthline-made" ? earthlineLogo : HandoutStudioLogo} alt="logo" fill={false} />
										</div>
										<h2 className={`text-xl md:text-3xl font-light leading-tight`}> {`We make things that stay forever.`} </h2>
									</div>
								</div>
								...
								{/* Back to top */}
								<div className="fixed bottom-30 right-6 bg-[#564F47] text-[#EDE8E4] rounded-lg p-4 shadow-lg" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}> ↑ </div>
