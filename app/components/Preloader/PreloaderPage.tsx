...
interface PreloaderPageProps { words: string[], caller?: string }

export default function PreloaderPage({words, caller}: PreloaderPageProps) {
	const [index, setIndex] = useState(0);
	const [dimension, setDimension] = useState({width: 0, height:0});

	useEffect( () => { setDimension({width: window.innerWidth, height: window.innerHeight}) }, [])
	...
	const curve = {
		initial: { d: initialPath, transition: {duration: 0.7, ease: [0.76, 0, 0.24, 1] as Easing} },
		exit: { d: targetPath, transition: {duration: 0.7, ease: [0.76, 0, 0.24, 1] as Easing, delay: 0.3} }
	}

	const opacity = { initial: { opacity: 0 }, enter: { opacity: 0.75, transition: {duration: 1, delay: 0.2} } }
