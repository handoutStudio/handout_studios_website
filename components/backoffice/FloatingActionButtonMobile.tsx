	...
	return (
		<Box sx={{ transform: 'translateZ(0px)', flexGrow: 1, zIndex: 100 }}>
			...
			<SpeedDial className={`${parentOpen ? 'bg-[#7c0104] dark:bg-white' : 'bg-transparent'} rounded-full m-2 p-0`} ariaLabel="SpeedDial tooltip example" sx={{ position: 'absolute', bottom: 16, right: 16, zIndex: 9999 }} icon={ <Avatar src={logo.src} alt="Remy Sharp" className={`bg-white rounded-full p-2 w-10 h-10`}>{}</Avatar> } onClose={handleParentClose} onOpen={handleParentOpen} open={parentOpen} FabProps={{ className:`m-1`, size:'small' }}>
				<Tooltip title="Catelogue" placement={nestedOpen ? "bottom" : "left"}>
					<SpeedDialAction
						className={``}
						...
						slotProps={{ fab: { className: `${['attributes', 'banners', 'products', 'categories', 'coupons'].filter((item => pathName.includes(item))).length > 0 ? '!bg-[#E20108] !text-white dark:!bg-white dark:!text-[#AF0106]' : '!text-[#7C0104] dark:bg-[#AF0106] dark:!text-white' } hover:!bg-[#E20108] hover:!text-white dark:hover:!bg-white dark:hover:!text-[#E20108] p-0 m-1`, size: 'small' } }}
				...
				</Tooltip>
				{
					pathsReturn.map((item: any) => (
						<Tooltip title={item.name} placement={nestedOpen ? "right" : "left"}>
							<SpeedDialAction
								className={``}
								key={item.name}
								icon={item.icon}
								onClick={() => handleOpenLink(item.path)}
								slotProps={{ fab: { className: `${pathName === item.path ? '!bg-[#E20108] !text-white dark:!bg-white dark:!text-[#AF0106]' : '!text-[#7C0104] dark:bg-[#AF0106] dark:!text-white' } hover:!bg-[#E20108] hover:!text-white dark:hover:!bg-white dark:hover:!text-[#E20108] p-0 m-1`, size: 'small' } }}
